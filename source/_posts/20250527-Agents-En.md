...
![](/images/notes/c414057773a4141be754c9b14f26cb6e539cfdb832ec4ab93bb2a43fe67a65d6.jpg)  
...
![](/images/notes/dc6180fdf4f4bd56752dce4fb0c46a763bab9a6915ffccb68678cc9d49a72bf6.jpg)  
...
<html><body><table><tr><td>Models</td><td>Agents</td></tr><tr><td>Knowledge is limited to what is available in their training data.</td><td>Knowledge is extended through the connection with external systems via tools</td></tr><tr><td>Single inference / prediction based on the user query. Unless explicitly implemented for the model, there is no management of session history or continuous context. (i.e. chat history)</td><td>Managed session history (i.e.chat history) to allow for multi turn inference /prediction based on user queries and decisions made in the orchestration layer.In this context,a 'turn'is defined as an interaction between the interacting system and the agent. (i.e.1incoming event/</td></tr><tr><td>No native tool implementation.</td><td>query and 1agent response) Tools are natively implemented in agent architecture.</td></tr><tr><td>No native logic layer implemented. Users can form prompts as simple questions or use reasoning frameworks (CoT, ReAct,etc.) to form complex prompts to guide the model in prediction.</td><td>Native cognitive architecture that uses reasoning frameworks like CoT, ReAct,or other pre-built agent frameworks like LangChain.</td></tr></table></body></html>  
...
d.  Action input: The model’s decision on what inputs to provide to the tool (if any) e.  Observation: The result of the action / action input sequence i.  This thought / action / action input / observation could repeat N-times as needed f.  Final answer: The model’s final answer to provide to the original user query  
...
1. Teaching the agent how to use the API endpoint using examples. 2.  Teaching the agent what arguments or parameters are needed to successfully call the API endpoint.  
...
import vertexai
import pprint
PROJECT_ID $\mathbf { \sigma } = \mathbf { \sigma }$ "YOUR_PROJECT_ID"
REGION $\mathbf { \tau } = \mathbf { \tau }$ "us-central1"
vertexai.init(project $\mathbf { \tau } = \mathbf { \tau }$ PROJECT_ID, location $\ c =$ REGION)
from vertexai.preview.extensions import Extension
extension_code_interpreter $\mathbf { \sigma } = \mathbf { \sigma }$ Extension.from_hub("code_interpreter")
CODE_QUERY $\mathbf { \sigma } = \mathbf { \sigma }$ """Write a python method to invert a binary tree in O(n) time."""
response $\mathbf { \tau } = \mathbf { \tau }$ extension_code_interpreter.execute( operation_id $\mathbf { \sigma } = \mathbf { \sigma }$ "generate_and_execute", operation_params $\mathbf { \sigma } = \mathbf { \sigma }$ {"query": CODE_QUERY} )
...
Generated Code:
class TreeNode: def __init__(self, $\mathsf { v a l } = \mathsf { \boldsymbol { \theta } }$ , left $\ c =$ None, right $\ c =$ None): self.val $\mathbf { \tau } = \mathbf { \tau }$ val self.left $\mathbf { \tau } = \mathbf { \tau }$ left self.right $\mathbf { \tau } = \mathbf { \tau }$ right  

# Python  

def invert_binary_tree(root): Inverts a binary tree. Args: root: The root of the binary tree. Returns: The root of the inverted binary tree. 1  

if not root: return None  

# Swap the left and right children recursively root.left, root.right $\mathbf { \sigma } = \mathbf { \sigma }$ invert_binary_tree(root.right), invert_binary_tree(root.left)  
...
root $\mathbf { \sigma } = \mathbf { \sigma }$ TreeNode(4)
root.left $\mathbf { \sigma } = \mathbf { \sigma }$ TreeNode(2)
root.right $\mathbf { \sigma } = \mathbf { \sigma }$ TreeNode(7)
root.left.left $\mathbf { \sigma } = \mathbf { \sigma }$ TreeNode(1)
root.left.right $\mathbf { \sigma } = \mathbf { \sigma }$ TreeNode(3)
root.right.left $\mathbf { \tau } = \mathbf { \tau }$ TreeNode(6)
root.right.right $\mathbf { \sigma } = \mathbf { \sigma }$ TreeNode(9)  

# Invert the binary tree inverted_root $\mathbf { \sigma } = \mathbf { \sigma }$ invert_binary_tree(root)  
...
1. A model outputs a Function and its arguments, but doesn’t make a live API call. 2.  Functions are executed on the client-side, while Extensions are executed on the agent-side.  
...
I’d like to take a ski trip with my family but I’m not sure where to go. In a typical prompt to the model, the output might look like the following: Sure, here’s a list of cities that you can consider for family ski trips:  

•  Crested Butte, Colorado, USA Whistler, BC, Canada Zermatt, Switzerland  
...
•  You are running asynchronous operations that can take more than a few seconds. These scenarios work well with function calling because it's an asynchronous operation. •  You want to run functions on a device that's different from the system producing the function calls and their arguments.  
...
model $\mathbf { \tau } = \mathbf { \tau }$ GenerativeModel("gemini-1.5-flash-001")
display_cities_function $\mathbf { \tau } = \mathbf { \tau }$ FunctionDeclaration.from_func(display_cities)
tool $\mathbf { \sigma } = \mathbf { \sigma }$ Tool(function_declarations $\ c =$ [display_cities_function])
message $\mathbf { \sigma } = \mathbf { \sigma }$ "I’d like to take a ski trip with my family but I’m not sure where
to go."
res $\mathbf { \tau } = \mathbf { \tau }$ model.generate_content(message, tools $\mathbf { \Sigma } = \mathbf { \Sigma }$ [tool])
...
•  Website content Structured Data in formats like PDF, Word Docs, CSV, Spreadsheets, etc. •  Unstructured Data in formats like HTML, PDF, TXT, etc.  
...
<html><body><table><tr><td></td><td>Extensions</td><td>Function Calling</td><td>Data Stores</td></tr><tr><td>Execution Use Case</td><td>Agent-Side Execution</td><td>Client-Side Execution</td><td>Agent-Side Execution Developer wants to</td></tr><tr><td></td><td>·Developer wants agent to control interactions with the APl endpoints Useful when leveraging native pre- built Extensions (i.e. Vertex Search, Code Interpreter, etc.) Multi-ocpalaning (i.eithendsent the outputs of the previous action / API call)</td><td>·Security or Authentication restrictions prevent the agent from calling an API directly Timing constraints or order-of-operations constraints that prevent the agent frmamtiking gi.Pl calch operations,human-in- the-loop review, etc.) APl that is not exposed to the internet,or non-accessible by Google systems</td><td>implement Retrieval Augmented Generation (RAG) with any of the following data types: Website Content from pre-indexed domains and URLs Structured Data in word Docs PDF, Spreadsheets,etc. Relational / Non- Relational Databases Unstructured Data in formats like HTML, PDF, TXT, etc.</td></tr></table></body></html>  
...
os.environ["SERPAPI_API_KEY"] $\mathbf { \sigma } = \mathbf { \sigma }$ "XXXXX"
os.environ["GPLACES_API_KEY"] $\mathbf { \sigma } = \mathbf { \sigma }$ "XXXXX"
@tool
def search(query: str): ""Use the SerpAPI to run a Google Search.""" search $\mathbf { \sigma } = \mathbf { \sigma }$ SerpAPIWrapper() return search.run(query)
@tool
def places(query: str): """Use the Google Places API to run a Google Places Query.""" places $\mathbf { \tau } = \mathbf { \tau }$ GooglePlacesTool() return places.run(query)
model $\mathbf { \sigma } = \mathbf { \sigma }$ ChatVertexAI(model $\ O =$ "gemini-1.5-flash-001")
tools $\mathbf { \sigma } = \mathbf { \sigma }$ [search, places]
query $\mathbf { \tau } = \mathbf { \tau }$ "Who did the Texas Longhorns play in football last week? What is the
address of the other team's stadium?"
agent $\mathbf { \tau } = \mathbf { \tau }$ create_react_agent(model, tools)
input $\mathbf { \sigma } = \mathbf { \sigma }$ {"messages": [("human", query)]}
for s in agent.stream(input, stream_mode $\ c =$ "values"): message $\mathbf { \sigma } = \mathbf { \sigma }$ s["messages"][-1] if isinstance(message, tuple): print(message) else: message.pretty_print()  

# Unset  

== Human Message = Who did the Texas Longhorns play in football last week? What is the address of the other team's stadium? ==== Ai Message ==== == Tool Calls: search Args: query: Texas Longhorns football schedule ===== Tool Message === = Name: search {...Results: "NCAA Division I Football, Georgia, Date..."} = Ai Message $\scriptstyle = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =$ The Texas Longhorns played the Georgia Bulldogs last week. Tool Calls: places Args: query: Georgia Bulldogs stadium = Tool Message Name: places {...Sanford Stadium Address: 100 Sanford...} == Ai Message The address of the Georgia Bulldogs stadium is 100 Sanford Dr, Athens, GA 30602, USA.  
