...
本文将深入解读 **Transformer² (Transformer-Squared)** 的核心设计。它提出了一种让 LLM 通过选择性地、实时地调整其内部参数来适应未见任务的新方法。其核心在于一种名为 **奇异值微调（Singular Value Fine-tuning, SVF）** 的新型 PEFT 技术，以及利用强化学习（RL）训练出的可组合”专家模块”，并通过巧妙的两阶段推理机制动态应用。
